    ...
    <el-row>
      ...
      {{ c }}
        ...
        <div v-else>
          <el-card class="box-card">
            ...
            <el-tooltip class="item" effect="dark" :content="o.errorMessage" placement="top-start" v-for="o in result" :key="o.id">
              <el-card shadow="hover" :style="conclusionBackgroundColor(o.statement)" type="success" class="text item">
      ...
      <el-col :span="18" :offset="3">
        <el-card class="box-card" v-if="CE != null">
          源代码
          <code-editor ref="editor" :code="this.code" :cm-mode="editorStyle" :read-only="true" :cm-theme="'monokai'"/>
        </el-card>
      </el-col>
    </el-row>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      conclusion: "ACCEPTED",
      localhostPath: "",
      languageType: this.$route.query.judgeData.languageType
    }
  },
  created() {
    ...
    this.$axios.post(
      "judge/judgement?id=" + this.$route.params.id,
      this.$route.query.judgeData
    ).then((data) => {
  ...
  computed: {
    ...
    editorStyle() {
      ...
      return mapNumToStyle[this.languageType]
    },
    codeLines() {
      if(this.code != null) {
